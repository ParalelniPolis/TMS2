    ...
    public function tryLogin($data) {
        ...
        //account is locked by bruteforce
            ...
            if ($unlockMailCheck[0] != null) return ['s' => 'error',
                ...
                'en' => 'Mail wit unblock was already sent - if you did\'t recieve anything, contact please our webmaster'];
            ...
            //...and send email
            $activeLink = [
                'cs' => ROOT.'/cs/unlockBrutforce/'.$randomHash,
                'en' => ROOT.'/en/unlockBrutforce/'.$randomHash];
            $subject = [
            ...
            $message = 'Hi! <br/>
                       ...
                       <br/>
                       Clicking on this link will unlock '.BRUTEFORCE_NUMBER_OF_ATTEMPTS.' more tries: <a href="'.$activeLink['en'].'">'.$activeLink['en'].'</a><br/>';
            $message .= '<br/><hr><br/>';
            $message .= 'Ahoj! <br/>
                       <br/>
                       ...
                       <br/>
                       Kliknutí na tento link ti odemkne dalších pět pokusů: <a href="'.$activeLink['cs'].'">'.$activeLink['cs'].'</a><br/>';
            //TODO fix multilingual emails
            $this->sendEmail(EMAIL, $data['login'], $subject['en'], $message);

            $udaje = ['sentUnlockBruteforce', $data['login'], 'mail with unlocking link is sent'];
