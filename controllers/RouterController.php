...
class RouterController extends Controller {
	...
	public function process($parameters) {
		...
		if ($userLanguage == false) {
			$this->messages[] = [
				's' => 'error',
				'cs' => 'Bohužel jsme nezrozpoznali tvůj preferovaný jazyk',
				'en' => 'Sorry, we didn\'t recognize your language'
			];
		...
		if (file_exists('controllers/'.$controllerClass.'.php'))
			$this->controller = new $controllerClass($this->language); else $this->redirect('error');
		...
		switch ($this->language) {
			case ('cs'): {
				$this->data['title'] = NAME.' - '.$this->controller->header['title']['cs'].' - v'.VERSION.' - Paralelní polis';
			...
			case ('en'): {
				$this->data['title'] = NAME.' - '.$this->controller->header['title']['en'].' - v'.VERSION.' - Paralell polis';
